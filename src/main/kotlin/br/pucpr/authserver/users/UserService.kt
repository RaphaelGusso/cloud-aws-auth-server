...
class UserService(
    ...
    fun insert(user: User): User {
        ...
        saved.avatar = avatarService.resolve(saved)
    ...
    /** Discards the current avatar and resolves a new one (Gravatar -> ui-avatars -> default). */
    fun resetAvatar(id: Long): String {
        val user = findById(id)
        user.avatar = avatarService.resolve(user)
        repository.save(user)
        log.info("Avatar of user $id was reset")
        return avatarService.urlFor(user.avatar)
    }
